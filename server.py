...
# 🗄️ MongoDB Setup (LockdownBot ফোল্ডারের জন্য)
...
def remove_token_from_db(identifier):
    ...
    try:
        ...
        print(f"[-] Error updating Database during removal: {e}")
...
def start_shield():
    ...
    # 🟢 ফ্রেশ মেসেজ (লগে শুধু এইটুকু দেখাবে)
    return jsonify({"message": f"Shield Activated for {username}!"}), 200
...
def stop_shield():
    ...
    # ১. ডাটাবেস থেকে টোকেন রিমুভ করবে
...
if __name__ == '__main__':
    ...
    if saved_tokens:
        print(f"[*] Found {len(saved_tokens)} saved tokens in Database. Auto-starting protection threads...")
        for token, username in saved_tokens.items():
            threading.Thread(target=run_bot_in_background, args=(token, username), daemon=True).start()
    else:
        print("[*] No previous tokens found. Standing by...")
